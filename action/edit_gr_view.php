...
$link='work.php?action=edit_gr_view';
if(isset($_GET['idr'])){
	$idr=$_GET['idr'];
	$link=$link.'&idr='.$idr;
	if(!empty($_POST)){
		...
		$curent_user->editviewgroup($idr,array_keys($_POST));
	}
}else{
	$idr=0;
	print_r($_POST);
...
<?php
$resultroll = mysql_query("SELECT * FROM `viewgroup` WHERE id='$idr'");
		if($myrol=mysql_fetch_array($resultroll)){
			echo '<h3><p align="center">Параметры роли '.$myrol['name'].'</p></h3>';
		}
		$resultpriv = mysql_query("SELECT * FROM `organization` ");
		echo '<table align="center">';
		while($mypriv=mysql_fetch_array($resultpriv)){
			if(QwestViewGroupFoID($mypriv['id'], $idr)){
